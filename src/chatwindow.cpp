...
void ChatWindow::onSetupAuthorizeActions() {
  ...
  if(abook_roster_cache.contains(persistent_uid)) {
    ...
    if(item->published() == "local-pending") {
      qDebug() << "menuAuthorize: request pending";
      ui->menuAuthorize->setEnabled(true);
      ui->actionAddFriend->setEnabled(false);
      ui->actionRemoveFriend->setEnabled(false);
      ui->actionAcceptFriendRequest->setEnabled(true);
      ui->actionRejectFriendRequest->setEnabled(true);
    } else if(item->published() == "yes") {
      qDebug() << "menuAuthorize: is_friend";
      ui->menuAuthorize->setEnabled(true);
      ui->actionAddFriend->setEnabled(false);
      ui->actionRemoveFriend->setEnabled(true);
      ui->actionAcceptFriendRequest->setEnabled(false);
      ui->actionRejectFriendRequest->setEnabled(false);
    } else {
      qDebug() << "menuAuthorize: hmm";
    }
  }
}
